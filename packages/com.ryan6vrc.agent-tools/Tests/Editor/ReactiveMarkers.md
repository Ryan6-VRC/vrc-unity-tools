...
The predicate gates on the **`nadena.dev.modular_avatar*` namespace** rather than on any type whose namespace merely contains `modular_avatar`. `RenderAvatarFreshnessTests` defines `modular_avatar_fixture.FakeShapeChanger`, a `MonoBehaviour` existing precisely to be matched by the production scan; the looser predicate is satisfied for `"ShapeChanger"` by that fixture alone, so it would stay green with MA absent or its real type renamed, and would turn the MA-absent skip into a false pass.
