...
Reflecting NDMF's filter registry instead of maintaining a list is unreachable, in two independent ways. The version anchor is load-bearing here — a null result about a vendor API expires when the vendor changes — so: measured against MA 1.18.7 and the NDMF on disk beside it.
