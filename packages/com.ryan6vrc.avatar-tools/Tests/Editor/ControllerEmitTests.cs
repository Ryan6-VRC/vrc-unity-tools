...
";
    ...
    // ---- compiled-from: provenance path ----------------------------------------------------------
    // The stamp lands in a .controller.meta that gets COMMITTED to a public repo, so it must carry no
    // machine path. Absolute input is NORMALIZED, never refused: compiling from an arbitrary filesystem
    // --root is how vrc-patterns/tools/gate.ps1 is built to work.

    [Test]
    public void Provenance_Under_The_Project_Is_Project_Relative()
    {
        var projectRoot = Path.GetDirectoryName(Application.dataPath).Replace('\\', '/');
        Assert.AreEqual("Assets/Agent/Scratch/x.yaml",
            ControllerEmit.ProvenanceSourcePath(projectRoot + "/Assets/Agent/Scratch/x.yaml"));
        Assert.AreEqual("Packages/com.ryan6vrc.patterns/_template/controller.yaml",
            ControllerEmit.ProvenanceSourcePath(projectRoot.Replace('/', '\\') + "\\Packages\\com.ryan6vrc.patterns\\_template\\controller.yaml"),
            "backslashes normalize too — a Windows path is the one that leaked");
    }

    [Test]
    public void Provenance_Outside_The_Project_Is_Repo_Relative()
    {
        // The gate's --root case: a checkout the Editor never mounted. The repo root is found by its .git,
        // so neither the machine path nor the checkout's own directory name reaches the stamp — the same
        // string comes out of a worktree slice as out of the main clone.
        var repo = Path.Combine(Path.GetTempPath(), "prov_" + System.Guid.NewGuid().ToString("N").Substring(0, 8)).Replace('\\', '/');
        Directory.CreateDirectory(repo + "/.git");
        Directory.CreateDirectory(repo + "/_template");
        try
        {
            Assert.AreEqual("_template/controller.yaml",
                ControllerEmit.ProvenanceSourcePath(repo + "/_template/controller.yaml"));
        }
    ...
    public void Provenance_With_No_Anchor_Falls_Back_To_The_Leaf()
    {
        ...
        Assert.AreEqual("controller.yaml", ControllerEmit.ProvenanceSourcePath(orphan + "/controller.yaml"),
            "an anchorless absolute path keeps only its leaf");
    }

    [Test]
    public void Provenance_Already_Relative_Is_Untouched()
    {
        Assert.AreEqual("Packages/com.x/e/controller.yaml",
            ControllerEmit.ProvenanceSourcePath("Packages/com.x/e/controller.yaml"));
        Assert.AreEqual("mem://fixture", ControllerEmit.ProvenanceSourcePath("mem://fixture"),
            "the in-memory fixture source is not a filesystem path");
        Assert.AreEqual("", ControllerEmit.ProvenanceSourcePath(null));
    }
}
