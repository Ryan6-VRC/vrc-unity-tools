...
| [`com.ryan6vrc.avatar-tools`](packages/com.ryan6vrc.avatar-tools) | Agent-callable avatar tools: package-graph inspection + a type-driven component-transplant kit (CopyComponents / RelocateComponents / GraftHierarchy over a shared core) + a clean-FX builder + clip/material utilities (expression-clip normalizer, HSVG splitter, material remap, duplicate+constrain — each with a menu/window door and an agent-callable `Run(...)`). | Yes (gated on `VRC_SDK_VRCSDK3`). |
