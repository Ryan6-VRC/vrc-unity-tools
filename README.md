...
|---|---|---|
| [`com.ryan6vrc.agent-tools`](packages/com.ryan6vrc.agent-tools) | The **inspection and verification harness** for AI-assisted (Claude Code) avatar workflows — read-only digests, gates, and renders that report on an avatar without changing it, so an agent can observe and prove rather than assert. Two project-hygiene doors here do write, both at the import door rather than on an avatar: `ImportPackage` (it imports) and `ConformImportSettings` (it corrects the importer settings that block an upload). Also home of the canonical `[AgentTool]` marker and the shared RunLog text conventions (`RunLogFormat`). | Yes (gated on `VRC_SDK_VRCSDK3`). |
